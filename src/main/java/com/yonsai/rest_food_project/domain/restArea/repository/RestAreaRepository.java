...
public interface RestAreaRepository extends JpaRepository<RestArea, Long> {
    ...
    // 랜덤 정렬
    @Query(value = "SELECT * FROM rest_area WHERE gasoline_price > 0 ORDER BY RAND()", 
           countQuery = "SELECT count(*) FROM rest_area WHERE gasoline_price > 0", 
           nativeQuery = true)
    Page<RestArea> findRandomAreas(Pageable pageable);
